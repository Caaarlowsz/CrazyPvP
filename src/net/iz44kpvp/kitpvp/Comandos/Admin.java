...
public class Admin implements CommandExecutor, Listener
{
    ...
    public void oaTrocar(final PlayerInteractEvent e) {
        final Player p = e.getPlayer();
        if (Admin.emAdmin.contains(p) && (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && p.getItemInHand().getType() == Material.ENDER_PEARL) {
            e.setCancelled(true);
    ...
    public void aoAutoSoup(final PlayerInteractEntityEvent e) {
        ...
        if (Admin.emAdmin.contains(p) && p.getInventory().getItemInHand().getType() == Material.MUSHROOM_SOUP) {
            if (Admin.aotestarautosoup.contains(p.getName())) {
                p.sendMessage(String.valueOf(String.valueOf(API.preffix)) + "§cVoc\u00ea j\u00e1 esta testando um player");
                return;
            }
            ...
            p.sendMessage(String.valueOf(String.valueOf(API.preffix)) + "§7Voc\u00ea est\u00e1 testando o auto-soup de: §b" + t.getDisplayName());
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(Main.plugin, (Runnable)new Runnable() {
                ...
                public void run() {
                    ...
                    p.sendMessage(String.valueOf(String.valueOf(API.preffix)) + "§7Quantidades de sopa(s) bebida(s): §b" + API.getItemNoInv(t, Material.BOWL));
                    p.sendMessage(String.valueOf(String.valueOf(API.preffix)) + "§7Quantidades de sopa(s) n\u00e3o bebida(s): §b" + API.getItemNoInv(t, Material.MUSHROOM_SOUP));
