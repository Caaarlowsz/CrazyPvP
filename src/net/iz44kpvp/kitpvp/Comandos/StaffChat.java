...
public class StaffChat implements Listener, CommandExecutor
{
    ...
    public void onChat(final PlayerChatEvent e) {
        ...
        if (this.isStaffChatting(p)) {
            Bukkit.broadcast(ChatColor.GRAY + "[§cStaff" + ChatColor.GRAY + "]§9 " + p.getDisplayName() + "§a \u27a1 " + "§f" + e.getMessage().replaceAll("&", "§"), "flame.staff");
            e.setCancelled(true);
        }
    }
            ...
            if (p.hasPermission("flame.staff")) {
                if (this.isStaffChatting(p)) {
                    ...
                    for (int length = (onlinePlayers = Bukkit.getOnlinePlayers()).length, i = 0; i < length; ++i) {
                        ...
                        if (this.isStaffChatting(p) && (online.hasPermission("flame.staff") || this.isStaffChatting(online))) {
                    ...
                    for (int length2 = (onlinePlayers2 = Bukkit.getOnlinePlayers()).length, j = 0; j < length2; ++j) {
                        ...
                        if (this.isStaffChatting(p) && (online.hasPermission("flame.staff") || this.isStaffChatting(online))) {
                            online.sendMessage(ChatColor.GRAY + "[§cStaff" + ChatColor.GRAY + "]§9 O Jogador: " + p.getDisplayName() + " §aEntrou no Chat da Staff");
                        }
                    }
                }
            }
